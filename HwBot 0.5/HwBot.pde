...
int leftborder = 55, rightborder = 650 ;
float linegap = 25 ; 
void settingup() {
  ...
  buttons.get(0).sliders.add( new Slider( "Size", 64, 110, 210, 20)) ;
  ...
  buttons.add( new Button( "Image", 220, 50, 40, 15) );
...
void draw() {
  ...
  if ( backgroundPage != null ) image(backgroundPage, backgroundPagex, backgroundPagey ) ; //, (backgroundPage.width)*zoomscale, (backgroundPage.height)*zoomscale ) ; 
  scale(zoomscale) ;
